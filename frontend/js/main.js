...
class TweetHeap {
  ...
  // Get all tweets in sorted order (by score)
  getSorted() {
    const result = [];
    const tempHeap = new TweetHeap();
    
    // Create a copy of the heap
    this.heap.forEach(tweet => tempHeap.insert(tweet));
...
class NewsFeed {
  ...
  async loadFeed() {
    try {
      ...
      const response = await fetch(`/feed?userId=${userId}`);
      ...
      const sortedFeed = feed; // Already sorted by score from the backend
      window.currentPosts = sortedFeed;
      return sortedFeed;
    } catch (error) {
