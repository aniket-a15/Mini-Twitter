...
app.get('/feed', (req, res) => {
  ...
  posts.forEach(post => {
    ...
    // 1. Own Post: +6 if post.userId === currentUser.id
    if (post.userId === userId) {
      score += 6;
      scoringFactors.ownPost = 6;
    }
    
    // 2. Followed Author: +5 if post.userId is in currentUser.following
    if (following.has(post.userId)) {
      score += 5;
      scoringFactors.followedAuthor = 5;
    }
    
    // 3. Genre Match: +1 to +5 based on number of matching genres
    if (postGenres.length > 0 && userGenres.length > 0) {
      ...
      if (matchCount > 0) {
        // Calculate score: +3 for an exact match, otherwise a base of 1 plus 0.5 for each additional match
        let genreScore = 0;
        
        if (matchCount === postGenres.length && matchCount === userGenres.length) {
          // Exact match (all genres match exactly)
          genreScore = 5;
          scoringFactors.exactGenreMatch = 5;
        } else {
          // Partial match - base score of 1 plus 0.5 for each matching genre
          genreScore = Math.min(4, 1 + (matchCount * 0.5));
    ...
    // 4. Genre Preference: Add points for each genre the user has engaged with
    postGenres.forEach(genre => {
      if (genre && currentUser.genrePreference[genre] > 0) {
        score += 1;
        scoringFactors.genrePreference = scoringFactors.genrePreference || 0;
        scoringFactors.genrePreference += 1;
      }
    });
    
    // 5. Liked Same Author: +3 if currentUser.authorAffinity[post.userId] > 0
    if (currentUser.authorAffinity[post.userId] > 0) {
      score += 3;
      scoringFactors.likedSameAuthor = 3;
    }
    
    // 6. Commented Same Author: +2 if the user has commented on that author before
    if (commentHistoryByAuthor[post.userId] > 0) {
      score += 2;
      scoringFactors.commentedSameAuthor = 2;
    }
    
    // 7. Comments in Genre: +1 for each genre the user has commented on
    let genreCommentScore = 0;
    postGenres.forEach(genre => {
      if (genre && commentHistoryByGenre[genre] > 0) {
        genreCommentScore += 0.5;
      }
    });
    
    if (genreCommentScore > 0) {
      const cappedScore = Math.min(2, genreCommentScore);
      score += cappedScore;
      scoringFactors.commentsInGenre = cappedScore;
    }
    
    // 8. Image Bonus: +1 if post.image is not null
    if (post.image) {
      score += 1;
      scoringFactors.imageBonus = 1;
    }
    
    // 9. Recency Bonus: up to +5 for freshness
    const postTime = new Date(post.timestamp);
    const now = new Date();
    const hoursDiff = (now - postTime) / (1000 * 60 * 60);
    if (hoursDiff < 1) {
      const recencyScore = 5;
      score += recencyScore; // Full bonus for posts less than 1 hour old
      scoringFactors.recencyBonus = recencyScore;
    } else if (hoursDiff < 5) {
      const recencyScore = 5 - (hoursDiff / 5) * 5; // Linear decay from 5 to 0 over 5 hours
      score += recencyScore;
      scoringFactors.recencyBonus = parseFloat(recencyScore.toFixed(1));
    }
    
    // 10. Post Engagement: Score based on likes, comments, and views
    const likesCount = post.likes ? post.likes.length : 0;
    const commentsCount = post.comments ? post.comments.length : 0;
    const viewsCount = post.views ? post.views.length : 0;
    
    // Individual engagement metrics with different weights
    const likesScore = likesCount * 1;        // +1 per like
    const commentsScore = commentsCount * 2;   // +2 per comment
    const viewsScore = viewsCount * 0.5;       // +0.5 per view
    ...
    if (viewsCount > 0) {
      scoringFactors.views = parseFloat(viewsScore.toFixed(1));
      score += viewsScore;
    }
    
    // 11. Friend-of-Friend: +2 if any user the current user follows also follows the post's author
    if (friendOfFriendMap[post.userId]) {
      score += 2;
      scoringFactors.friendOfFriend = 2;
    }
    
    // 12. Reciprocity: +1 if the author follows the current user back
    if (reciprocityMap[post.userId]) {
      score += 1;
      scoringFactors.reciprocity = 1;
    }
    
    // 14. Negative Feedback Penalty: -5 if the user previously marked this author or genre "not interested"
    let negativeFeedbackScore = 0;
    
    if (negativeAuthorFeedback.has(post.userId)) {
      negativeFeedbackScore -= 5;
    }
    
    // Check for negative feedback on any of the post's genres
    postGenres.forEach(genre => {
      if (genre && negativeGenreFeedback.has(genre)) {
        negativeFeedbackScore -= 2;
      }
    });
  ...
  // 13. Discovery Boost: Add +2-4 for 1-2 randomly chosen posts from authors outside following
  const nonFollowedPosts = posts.filter(post => 
    post.userId !== userId && !following.has(post.userId)
  );
  
  if (nonFollowedPosts.length > 0) {
    // Select 1-2 random posts for discovery boost
    const numDiscovery = Math.min(2, nonFollowedPosts.length);
    ...
    discoveryPosts.forEach(post => {
      ...
      if (postIndex >= 0) {
        // Add discovery boost to the score
        const boostValue = 2 + Math.floor(Math.random() * 3); // +2 to +4 boost
  ...
  // Get top 90 posts from the heap
  const topPosts = postsHeap.getTopN(90);
